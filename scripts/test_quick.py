...
async def main() -> int:
    ...
    print(
        f"  Possession: H{ma.home_team.possession_pct:.1f}% / A{ma.away_team.possession_pct:.1f}%"
    )
    for t in ["home", "away"]:
    ...
    for p in top:
        print(
            f"  Track {p.track_id:4d}: {p.distance_covered_m:5.0f}m, max={p.max_speed_kmh:5.1f} km/h"
        )
    return 0
