...
def detect_anomalies(events: list[dict], match_duration_min: float = 90.0) -> AnomalyReport:
    anomalies: list[dict] = []
            ...
            anomalies=[
                {
                    "type": "no_events",
                    "severity": "high",
                    "description": "No events in match data",
                    "event_id": None,
                }
            ],
            score=0.0,
...
def compute_data_quality_score(events: list[dict], match_duration_min: float = 90.0) -> float:
    if not events:
        return 0.0
