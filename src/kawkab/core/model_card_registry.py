...
register_model_card(ModelCard(
    ...
    input_features=["distance_m", "angle_deg", "placement_x", "placement_y", "shot_speed", "body_part"],
    ...
    known_limitations=["Placement is approximated from freeze-frame data when absent",
                       "No keeper-positioning context", "Shot speed default applied when unmeasured"],
    tested_range="Distance 1-40m, placements across the goal mouth, trained vs heuristic parity tests",
    failure_modes=["Very close-range headers can be overrated", "Deflected shots treated as clean strikes"],
    test_count=0,
))
...
register_model_card(ModelCard(
    ...
    input_features=["pass_end_x", "pass_end_y", "shot_distance_m", "shot_angle_deg", "body_part"],
    ...
    known_limitations=["No receiver-positioning context", "Does not separate created-chance quality from finisher skill"],
...
register_model_card(ModelCard(
    ...
    known_limitations=["Counts opponent passes only while ball is in play in the defined zone",
...
register_model_card(ModelCard(
    ...
    known_limitations=["Needs reliable player identity (fragmented tracking merges nodes)",
...
register_model_card(ModelCard(
    ...
    known_limitations=["Coarse zones lose within-zone context", "No game-state leverage adjustment"],
    tested_range="All wired action types, per-player aggregation",
    failure_modes=["Players with unusual positional roles compare poorly to league-average zones"],
    test_count=0,
))
...
register_model_card(ModelCard(
    ...
    known_limitations=["Requires complete event chains; gaps break the differential",
...
register_model_card(ModelCard(
    ...
    known_limitations=["Frame-rate dependent (works best on 25Hz vendor tracking)",
...
register_model_card(ModelCard(
    ...
    known_limitations=["Weights are hand-tuned, not learned", "No crowd/game-state psychological term"],
...
register_model_card(ModelCard(
    ...
    known_limitations=["Direction assumption: attacks toward x=105 (needs single attacking frame)",
...
register_model_card(ModelCard(
    ...
    known_limitations=["Delivery flight quality (inswing/outswing) not modeled",
